...
class ApiController extends AbstractRestfulController {
    ...
    public function __construct($container) {
        Logger::info("ApiController", __LINE__, 'Building ApiController (AJAX).');
    ...
    public function onDispatch(MvcEvent $e) {
        Logger::info("ApiController", __LINE__, "" . $e->getParam("request")->getRequestUri());
        try {
            return parent::onDispatch($e);
        } catch (Exception $exc) {
            Logger::info("ApiController", __LINE__, $exc->getTraceAsString());
        }
    }
    ...
    public function getList() {
        ...
        Logger::info("ApiController", __LINE__, 'ApiController Ajax called.');
    ...
    function saveItemPriceOverride($record, $rowIndex) {
        try {
            $this->transaction();
            Logger::info("ApiController", __LINE__, 'Calling saveItemPriceOverride');
            ...
            Logger::info("ApiController", __LINE__, 'ItemPriceOverride saved.');
            return new JsonModel(array(
                'success' => true, 'index' => $rowIndex, 'overrideprice' => $record->getOverrideprice()
            ));
        } catch (Exception $e) {
            $this->entityManager->getConnection()->rollBack();
            Logger::info("ApiController", __LINE__, strval($e));
            return new JsonModel(array(
                'success' => false,
            ));
        }
    ...
    function setLatestOverridePriceInactive($customerid, $salespersonusername, $productid, $rowIndex) {
        ...
        Logger::info("ApiController", __LINE__, 'find ItemPriceOverride salesperson: ' . $salespersonusername . " ProductId: " . $productid . " CustomerId: " . $customerid);
        ...
        Logger::info("ApiController", __LINE__, 'ItemPriceOverride set inactive.');
    ...
    protected function overridePrice($id) {
        //log this action
        Logger::info("ApiController", __LINE__, 'Saving overrideprice: ' . $this->parameter('overrideprice') . '.');
    ...
    private function saveRowPlusItemsPage($id, $rowIndex) {
        ...
        try {
            ...
            $this->entityManager->flush();
            $this->entityManager->getConnection()->commit();
            Logger::info("ApiController", __LINE__, 'RowPlusItemsPage saved.');
            return new JsonModel(array(
                'success' => true,
                'index' => $rowIndex,
    ...
    protected function getTable() {
        Logger::info("ApiController", __LINE__, 'Retrieving ' . $this->pricingconfig['by_sku_object_items_controller'] . '.');
        ...
        if ($json && array_key_exists($this->pricingconfig['by_sku_object_items_controller'], $json)) {
            ...
            $restcallitemsmerged = $this->tableService->_filter($json[$this->pricingconfig['by_sku_object_items_controller']], $customerid);
        } else {
            Logger::debug("ApiController", __LINE__, 'No ' . $this->pricingconfig['by_sku_object_items_controller'] . ' items found.');
        }
        return new JsonModel(array(
    ...
    protected function select() {
        ...
        if (empty($userinplay)) {
            Logger::info("ApiController", __LINE__, "UserinPlay is null!");
        }
        if (!empty($ids)) {
            foreach ($ids as $id) {
                Logger::info("ApiController", __LINE__, 'Selecting ' . $id);
